...
export function ReportsPage() {
  ...
  const [machineryReportList, setMachineryReportList] = useState<Pick<Machinery, 'id' | 'machineryName' | 'machineryType' | 'plateNumber' | 'driverName' | 'status'>[]>([]);
  const [includeOutOfService, setIncludeOutOfService] = useState(true);
  const [machineryListPdfLoading, setMachineryListPdfLoading] = useState(false);
  ...
  const handleMachineryReportDownloadPdf = useCallback(async () => {
    if (filteredMachineryList.length === 0) return;
    setMachineryListPdfLoading(true);
    try {
      const blob = await pdf(
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `machinery-list-report-${format(new Date(), 'yyyy-MM-dd')}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      toast.success('Machinery list PDF downloaded');
    } catch (err) {
      console.error('PDF generation failed:', err);
      toast.error('Failed to generate machinery list PDF');
    } finally {
      setMachineryListPdfLoading(false);
    }
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                size="sm"
                className="h-8"
                onClick={handleMachineryReportDownloadPdf}
                disabled={filteredMachineryList.length === 0 || machineryListPdfLoading}
              >
                {machineryListPdfLoading ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" />
                ) : (
                  <FileDown className="h-3.5 w-3.5 mr-1" />
                )}
                {machineryListPdfLoading ? 'Generating...' : 'Download PDF'}
              </Button>
              <Button
                variant="outline"
                size="sm"
